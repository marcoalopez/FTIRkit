...
def plot_crystal_axes(euler_angles_deg):
    ...
    # Estimate the position of vector E prime
    Eprime_vector = _project_vector_onto_plane(E_vector, a_axis, b_axis)
...
def _generate_arc_points(
    ...
    # Calculate the angle between the vectors in a counterclockwise direction
    angle = calculate_counterclockwise_angle(vector_1, vector_2)
...
def calculate_counterclockwise_angle(
    vector_1: np.ndarray,
    vector_2: np.ndarray,
    normal: np.ndarray,
) -> float:
